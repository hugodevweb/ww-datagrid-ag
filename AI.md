...
***Simplified Mode:***
- `viewMode`: `'auto' | 'simplified'` - Default `'auto'`. Set to `'simplified'` to render the same grid (all features available at runtime) while the editor hides advanced/long-tail settings — grouping, kanban/calendar, infinite scroll, column chooser, advanced filter builder, view-state variables, editing/performance/localization knobs, and most styling sections — leaving a curated panel: Mode, Data Source, Search, Columns, Pagination, Selection, and a small Style section. It only affects the editor configuration surface, not runtime behavior.
