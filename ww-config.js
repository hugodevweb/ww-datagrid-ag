export default {
  ...
  properties: {
    ...
    viewConfiguration: {
      ...
      bindingValidation: {
        type: "object",
        tooltip: "View configuration object: { sizes: {colId: width}, filters: {}, sorting: [{colId, sort}], columnsOrder: [colId], hiddenColumns: [colId], grouping: { columnId, order, showUnassigned }, kanban: { groupBy: string|null, cardFields: [field], order: [groupValue], showUnassigned: boolean, hiddenGroups: [groupValue] }, calendar: { dateField: string|null, eventFields: [field], colorByField: string|null, timeframe: 'day'|'week'|'month'|'year'|'custom', anchorDate: 'YYYY-MM-DD', customStart: 'YYYY-MM-DD'|null, customEnd: 'YYYY-MM-DD'|null, weekStartsOn: 0|1 } }. The grouping.columnId / kanban.groupBy must reference a select-type column. cardFields / eventFields are capped at 5 fields. calendar.dateField must reference a dateString or dateTime column; calendar.colorByField must reference a select column. kanban.hiddenGroups is the list of group values hidden from the kanban board. Datagrid group collapsed state is tracked separately (per view id) in its own WeWeb variable. Empty values ({} or []) are ignored.",
      },
      propertyHelp: {
        tooltip: "When this configuration changes, all settings will be applied to the grid/kanban/calendar, overriding user modifications. Structure: { sizes: {}, filters: {}, sorting: [], columnsOrder: [], hiddenColumns: [], grouping: { columnId, order, showUnassigned }, kanban: { groupBy, cardFields, order, showUnassigned, hiddenGroups }, calendar: { dateField, eventFields, colorByField, timeframe, anchorDate, customStart, customEnd, weekStartsOn } }. Set grouping.columnId / kanban.groupBy to a select-column field to split records into colored groups. kanban.cardFields / calendar.eventFields are the ordered list of fields shown (max 5; calendar requires at least 1 — first field is the event title). calendar.dateField is the date/timestamp column used to place events; calendar.colorByField (optional) colors events from a select column's option colors. Empty values ({} or []) are gracefully ignored and won't affect the current state.",
      },
      /* wwEditor:end */
    },
    viewEditedVariableId: {
